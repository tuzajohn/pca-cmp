...
public class CrbReportService
{
    ...
    private static string WriteExcel(List<CrbOutputRow> rows, List<string> unmatched, string storageRoot)
    {
        ...
        string[] headers =
        {
            ...
            "Salary", "Terms", "Active", "Statutory", "Allowance",
            "Deductions", "Stanbic", "Affordability", "Notes"
        };
        ...
        for (int i = 0; i < rows.Count; i++)
        {
            ...
            ws.Cells[row, 7].Value  = r.Terms;
            ws.Cells[row, 8].Value  = r.IsActive;
